...
> ★ 走 HTTPS REST + IRSA，跟 gRPC 無關。
> ★ 看 `/data` 目錄有新 block 就 upload，**不問 Prometheus**。

**3. 充當「最近 2h 資料」的查詢端點**

> ★ 不是 proxy 去問 Prometheus。
> ★ 是自己讀 `/data` 上**還沒被 upload / 還在 WAL** 的資料。
...
> ★ 這些**都不是 metric 資料**。Metric 資料 100% 透過共享 PVC 拿。
...
| 寫法 | 意思 |
|---|---|
| ❌ 讀 Prometheus | 模糊 — 會被當成 API 呼叫 |
| ✅ 讀 Prometheus 寫到 `/data` 的 TSDB 檔案 | 精確 — 透過 PVC，不是 API |
