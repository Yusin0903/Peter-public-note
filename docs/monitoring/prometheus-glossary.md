...
這裡 `pod` 這個 label 的值**變了**(`abc` → `xyz`)。對 Prometheus 來說,這是**兩條完全不同的 row**,因為 label 組合不同:

- 舊的 `pod="abc"`:停止寫入,但**不會馬上從 head 消失**,還會留著(下面講多久)
- 新的 `pod="xyz"`:全新一條,head_series +1
### 那「死掉的舊 series」多久才從 head_series 消失

這運作上容易搞混的一塊。一條停止寫入的 series,**不會立刻**從 `head_series` 數字裡消失,它會:
...
所以一條 deploy 換掉的舊 pod series,可能還會在 `head_series` 數字裡**多賴著最多約 2 小時**,才真正消失。這就是為什麼高頻部署的環境, head_series 會被一堆「已經死了但還沒清掉」的舊身分墊高。
